...
def header(ld: LeftDrawer, rd: RightDrawer):
	with ui.header().classes('fixed max-sm:hidden flex-nowrap bg-accent m-0 px-3 py-2 border-none items-center'):
		main_menu(ld, rd)
		ui.space().classes()
		with filter_visibility(paths='/', classes='max-md:hidden'):
			month_filter_ui()
