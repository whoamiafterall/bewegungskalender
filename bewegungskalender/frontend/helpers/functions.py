...
@contextmanager
def filter_visibility(classes: str | None = None, paths: Tuple[str] = ('/', 'karte')):
	with ui.element().bind_visibility_from(ROUTER, "current_page",
	                                       backward=lambda
			                                       e: e is not None and e.path in paths) as wrapper:
		wrapper.classes(classes)
		yield wrapper


def loading(page_label: str = '', timeout: float = 0.5):
	ui.notification(f"Lade {page_label}...", position='center', type='ongoing', spinner=True, timeout=timeout)


def opacity(color: str, percentage: int = 50):
